...
- [x] Bot scope + polish (2026-08-09): `AgentLoopService.buildSystemPrompt` now constrains the bot to the product's domain (deflect general-knowledge/off-topic questions, guide users back to what the tools can do), forbids leaking internal tool names/operation IDs to users (natural language only), and requires ASCII hyphens in identifiers. Incoming messages are normalized in `TelegramUpdatesProcessor` (`normalizeText` — typographic dashes like `–`/`‑`/soft hyphen/`−` become `-`) before they reach the LLM, confirmation judge, or session history. The processor shows Telegram's `typing` chat action (`sendChatAction`, re-sent every 4s) while the agent pipeline runs, cleared in a `finally`. Unit-tested.
...
- [ ] Webhook `secret_token` (keep in view): Telegram recommends setting a `secret_token` at `setWebhook` time and validating the `X-Telegram-Bot-Api-Secret-Token` header per request. Today the webhook is authenticated only by the high-entropy bot token in the URL (`/telegram/webhook/{botToken}`), which can leak via proxy/server logs. Add: store `secret_token` encrypted at rest alongside `botToken`, pass it in `setWebhook`, verify it in the webhook handler (reject with 401 on mismatch), and keep it masked like `authHeaderValue`.
...
- Agent loop fallback rule: retryable LlmError (429/5xx/network) OR empty reply on the primary model → retry with the fallback model. `LlmService` parses OpenRouter's `Retry-After` header into `LlmError.retryAfterSeconds`, and `AgentLoopService.callModel` retries the **fallback** up to 2 attempts with backoff (`retryAfterSeconds` when present, capped at 10s, else 1s) so free-tier rate-limit windows don't dead-end every message. Non-retryable errors (4xx other than 429) still fail fast.
...
- `src/mock` serves four self-contained mock companies. Each is `{ slug, name, blurb, spec, seed, routes, samplePrompts }` in `mock-data.ts`; `MockService` is a generic dispatcher (`:name` path patterns → params, in-memory `seed` state that persists across calls until restart). `createMockApis()` deep-clones seeds so every service instance (and every jest test) gets isolated state. `MockContext` now carries `headers` (passed through from the Express request), so mocks can enforce per-request auth.
  ...
  - `auth` — Ledger & Co Bank: **per-user authentication**. `POST /login` (no security) with `{ username, password }` returns a bearer token (test users: `alex`/`pine42`, `june`/`sable77`); every other endpoint requires `Authorization: Bearer <token>` (or the bare token) and 401s otherwise. Endpoints: `/me`, `/accounts`, `/accounts/{accountId}` (with transactions), `/transactions?accountId=`, `POST /transfers` (from/to/amount, insufficient-funds → 400). Purpose: surface how Lyrebird handles authenticated-user APIs — the current agent only injects one static service-level auth header, so a per-user token from login is **not** yet attached to subsequent tool calls (known gap, next feature).
...
- All specs except `auth` declare the `X-API-Key` apiKey security scheme at document level, so the tool executor exercises its auth-injection path (the mock ignores the header value). `auth` declares a `bearer` http scheme instead (`Authorization` header), with `POST /login` overriding it via `security: []`.
...
- **A reply is always sent** (fixed 2026-08-09): the agent loop no longer throws when it hits `MAX_ITERATIONS` (5) — it returns a graceful "I had trouble completing that" reply. The processor wraps the whole pipeline in try/catch (`telegram-updates.processor.ts`): if anything throws, it logs the error and sends "Something went wrong on my end" instead of silently dropping the message (which previously showed the typing bubble with no reply).
