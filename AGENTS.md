...
└── frontend/   # Vite + React + TS + Tailwind v4 — landing page + React Flow dashboard
    └── src/
        ...
        ├── pages/        # Dashboard (tenants list), TenantDetail (services + map)
        ├── api/client.ts # axios instance + typed endpoints (tenantsApi) + getApiError
        └── lib/spec.ts   # countEndpoints / hostOf / maskToken helpers
```
...
- [x] Frontend dashboard (React Flow web app): routes `/app` (tenant list, create tenant) and `/app/tenants/:tenantId` (service list, create/delete service, SystemMap graph). Axios client in `src/api/client.ts` (base URL from `VITE_API_BASE_URL`, default `http://localhost:3000`; optional `X-Admin-Key` from `localStorage['lyrebird_admin_key']`). Dashboard pages are `React.lazy` code-split so the landing bundle stays lean. Build + oxlint clean.
...
- Frontend app routes: `react-router-dom` BrowserRouter in `App.tsx` (`/` landing, `/app` dashboard, `/app/tenants/:tenantId` detail). Dashboard pages are `React.lazy`-loaded. All HTTP goes through `src/api/client.ts` (axios, base URL from `VITE_API_BASE_URL`, default `http://localhost:3000`); it attaches `X-Admin-Key` from `localStorage['lyrebird_admin_key']` (editable in the dashboard header). React Flow graphs live in `src/components/dashboard/SystemMap.tsx` (custom brand-styled nodes, `@xyflow/react`).
